...
- Issued-key basic market truth / structure: FX indicators/session, FX `fx levels`, `fx opening-range`, `fx price-action`, FX `fx volume-profile` and `fx volume-profile-pack` GC futures proxy evidence, FX `fx options-proxy` support-only ETF options proxy evidence, FX `fx positioning` provider-availability contract, `macro event-window` objective scheduled-event context, authenticated crypto market depth/trades/session/indicators, stock context-adjacent reads, stock `stocks imbalance-window` auction imbalance rolling window, stock `filings --provider sec` and `stocks facts` official SEC reads, and stock research inputs such as transcripts, analyst estimates, ratings, price targets, ratios, and key metrics may fit Free quota when not explicitly premium-gated.
- Stock research MVP A: `stocks transcripts`, `stocks analyst-estimates`, `stocks ratings`, `stocks price-targets`, `stocks ratios`, `stocks key-metrics`, `stocks extended-hours`, and `stocks research-context` expose neutral read-only evidence surfaces.
